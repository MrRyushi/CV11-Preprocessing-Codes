...
class MyDataset(object):
    ...
    def calculate_class_weights(self):
        ...
        # Compute sampler weights (inverse of class frequency)
        sampler_weights = {label: 1.0 / count for label, count in label_counts.items()}
        # Apply weights to each sample in dataset
        sample_weights = [sampler_weights[self.list[i][1]] for i in range(len(self.list))]
    ...
    def load_data(self, filename):
        ...
        try:
            if filename.endswith('npz'):
                return np.load(filename)['frames'] # old code: 'data'
    ...
    def _get_boundary(self, filename, raw_data):
        ...
        utterance_duration = float( info[0].split(' ')[1] ) # old code: info[4]
...
def pad_packed_collate(batch):
    ...
    if use_boundary:
        # added: ---------------------
        max_boundary_len = max([boundary.shape[0] for boundary in boundaries_tuple])
        # --------------------------------------
        boundaries_np = np.zeros((len(boundaries_tuple), max_boundary_len))
        
        for idx in range(len(data_np)):
            # added: -------------------------------
            boundary_len = boundaries_tuple[idx].shape[0]
            # check if padding is needed
            if boundary_len < max_boundary_len:
                # pad with zeros at the end to make length max_boundary_len
                padded_boundary = np.pad(boundaries_tuple[idx], (0, max_boundary_len - boundary_len), mode='constant')
                boundaries_np[idx] = padded_boundary
            else:
                # if already length max_boundary_len, just copy
            # --------------------------------------
